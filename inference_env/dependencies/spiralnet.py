...
################################################################################################################
# WORKS most up to date refactored version , testing jit compile WORKS
class SpiralnetEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, latent_channels,
        ...
        for idx in range(len(out_channels)):
            if idx == 0:
                self.en_layers.append(nn.Linear(in_channels *  self.spiral_indices[idx].size(1), out_channels[idx]))
                   #SpiralEnblock(in_channels, out_channels[idx],
                   #               self.spiral_indices[idx]))
            else:
                self.en_layers.append(nn.Linear(out_channels[idx - 1] *  self.spiral_indices[idx].size(1), out_channels[idx]))
    ...
    def pool(self, x, trans, dim: int = 1):
        indices = trans._indices()
        row = indices[0]
        col = indices[1]
        value = trans._values().unsqueeze(-1)
        out = torch.index_select(x, dim, col) * value
        size = list(x.size())
        size[dim] = trans.size(0)
        output = torch.zeros(size, device=x.device, dtype=x.dtype)
        output.scatter_add_(dim, row.unsqueeze(-1).expand_as(out), out)
        return output
    ...
    def forward(self, x):
        for i, layer in enumerate(self.en_layers):
            if i != len(self.en_layers) - 1:
                ...
                n_nodes, _ = self.spiral_indices[i].size()
                if x.dim() == 2:
                    x = torch.index_select(x, 0, self.spiral_indices[i].view(-1))
                    x = x.view(n_nodes, -1)
                elif x.dim() == 3:
                    bs = x.size(0)
                    x = torch.index_select(x, self.dim, self.spiral_indices[i].view(-1))
                    x = x.view(bs, n_nodes, -1)
                else:
                    raise RuntimeError('x.dim() is expected to be 2 or 3, but received {}'.format(x.dim()))

                x = F.elu(layer(x))
                x = self.pool(x, self.down_transform[i])
...
def hands_faces():
    return [(0, 1, 5), (0, 1, 2), (0, 1, 3), (0, 1, 4), (0, 5, 9),
               (0, 9, 13), (0, 13, 17), (0, 5, 6), (0, 5, 7), (0, 5, 8),
                (0, 9, 10), (0, 9, 11), (0, 9, 12), (0, 13, 14), (0, 13, 15),
                (0, 13, 16), (0, 17, 18), (0, 17, 19), (0, 17, 20), (0, 5, 8), (0, 9, 12)]

def instantiate_spiralnet(nr_layers = 8, output_dim = 16, motion = 'hands'):    
    ...
    return model, spiral_indices_list, down_transform_list
