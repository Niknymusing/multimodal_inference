...
from scipy.signal import firwin, kaiserord #,kaiser, kaiser_beta,
...
def get_qmf_bank(h, n_band):
    ...
    n_band: int
        number of sub-bands
    """
    k = torch.arange(n_band).view(-1, 1)
    N = h.shape[-1]
    t = torch.arange(-(N // 2), N // 2 + 1)
    p = torch.pow(-1, k) * torch.pi / 4
    mod = torch.cos((2 * k + 1) * torch.pi / (2 * n_band) * t + p)
    hk = 2 * h * mod
    return hk
