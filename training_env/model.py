...
class RWKV(pl.LightningModule):
    def __init__(self, args):
        ...
        self.controller_network = nn.ModuleList([Block(args, i) for i in range(args.n_layer)])

        self.output_network = nn.ModuleList([Block(args, i) for i in range(args.n_layer)])

        self.layer_norms = [nn.LayerNorm(args.n_embd).cuda()  for _ in range(self.n_features)]
        ...
        self.training_mode = 'init_representation_learning'
    ...
    def forward(self, x, model_idx = None):
        ...
        emb = self.layer_norms[model_idx](x).squeeze(0)#model.ln_out(x)
    ...
    def apply_masks(self, x):
        ...
        for mask in self.masks:
            expanded_mask = mask.expand(L, -1, -1)  # Expand the mask to match the input batch size
            masked_outputs.append(x * expanded_mask)  # Element-wise multiplication
        return masked_outputs
    ...
    def compute_mine_means(self, joint_subsequences, marg_subsequences):
        ...
        with torch.no_grad():
            ...
            mean_joint = torch.mean(torch.stack(joint_subsequences))
            mean_marg = torch.mean(torch.stack(marg_subsequences))

        mine_value = - mean_joint + torch.log(mean_marg + eps)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ti = time.time()
        # Unpack the batch
        if self.training_mode == 'init_representation_learning':
            try:
                ...
                for idx in range(self.n_features): # len of self.feature_indices list
                    ...
                    loss = self.compute_mine_means(joint_subsequences, marg_subsequences)#self.compute_mine(joint_mean, marg_mean)
                   
                    """if torch.isnan(loss) or torch.isinf(loss):
                            best_model_path = self.trainer.checkpoint_callback.best_model_path
                            if best_model_path:
                                self.load_state_dict(torch.load(best_model_path)['state_dict'])
                                print("NaN/Inf detected in loss, reverting to best checkpoint at:", loss)
                                # Consider skipping the rest of the step
                                return {'loss': loss, 'skip_step': True}
                            else:
                                print("NaN/Inf detected in loss, but no checkpoint available", loss)
    ...
    # Optionally log values here
                    ...
                    print('batch_len =', batch_len, ' nr of chunks for seq len', seq_len, 'and feature ', frequency_feature_idx, ', loss ===================' , loss)
        ...
        if self.training_mode == 'init_representation_learning':
            try:
                ...
                for idx in range(self.n_features): # len of self.feature_indices list
                    ...
                    # some extra pre-cautions (besides gradient-clipping in mine networks), if mine values goes unbounded
                    val_loss = self.compute_mine_means(joint_subsequences, marg_subsequences)#self.compute_mine(joint_mean, marg_mean)
...
"""
        ...
        for n in self.state_dict():
            ...
            if 'encoder' in n or 'mine' in n or 'accumulated_means' in n or 'count' in n:
                continue
            p = self.state_dict()[n]
            shape = p.shape
